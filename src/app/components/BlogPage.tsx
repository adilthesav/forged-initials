...
// Maps Supabase snake_case rows to the BlogPost interface
...
function mapRow(row: any): BlogPost {
  return {
    ...
    excerpt: row.excerpt || '',
    content: row.content || '',
    category: row.category || 'Style Tips',
    tags: row.tags || [],
    date: row.date || '',
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-50/30 via-white to-stone-50/30">
      ...
      <div className="container mx-auto px-4 sm:px-6 max-w-6xl">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 mb-8">
          {[...new Set(['All', ...posts.map(p => p.category)])].map(cat => (
            <button
              key={cat}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
          {filtered.map(post => (
            <PostCard key={post.id} post={post} onClick={() => goToPost(post)} />
          ))}
        </div>
