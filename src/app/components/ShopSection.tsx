...
function StockBadge({ qty }: { qty: number }) {
  ...
  if (qty <= 10) return (
    <span className="absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800">{qty} left</span>
  );
  return (
    <span className="absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">In Stock</span>
  );
}

function ProductCard({ product, onBuy, buying }: { product: Product; onBuy: (p: Product) => void; buying: boolean }) {
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden border border-stone-100 hover:border-amber-200 transition-all duration-300 hover:shadow-xl hover:shadow-amber-900/5 hover:-translate-y-1 flex flex-col">
      <div className="relative aspect-square overflow-hidden bg-stone-50">
        ...
        <span className="absolute top-2 left-2 text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full capitalize"
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        {product.description && (
          <p className="text-stone-500 text-xs sm:text-sm leading-relaxed line-clamp-2 mb-3 flex-1">
            {product.description}
          </p>
        )}
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-stone-50">
          ...
          <button
            ...
            style={soldOut ? { background: '#e5e7eb', color: '#9ca3af' } : {
              background: 'linear-gradient(135deg,#c9a84c,#e8c96a)',
              color: '#2a1800',
              boxShadow: '0 2px 10px rgba(201,168,76,0.3)',
            }}
          >
            {buying ? 'Loading…' : soldOut ? 'Sold Out' : 'Buy Now'}
          </button>
        </div>
      </div>
    </div>
...
export function ShopSection() {
  ...
  const availableCats = CATS.filter(c => c === 'All' || products.some(p => p.category === CAT_MAP[c]));
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {filtered.map(product => (
              <ProductCard key={product.id} product={product} onBuy={handleBuy} buying={buyingId === product.id} />
            ))}
          </div>
        )}
