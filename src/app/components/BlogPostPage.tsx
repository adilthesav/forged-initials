...
function mapRow(row: any): BlogPost {
  return {
    ...
    date: row.date || '', readTime: row.read_time || 3,
    coverImage: row.cover_image || '', coverAlt: row.cover_alt || '',
    seoTitle: row.seo_title || row.title,
    seoDescription: row.seo_description || '', seoKeywords: row.seo_keywords || '',
  };
}
...
export function BlogPostPage() {
  ...
  const [post, setPost] = useState<BlogPost | null>(BLOG_POSTS.find(p => p.slug === slug) || null);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then(r => r.json())
      .then(data => { if (Array.isArray(data) && data.length > 0) setPost(mapRow(data[0])); })
